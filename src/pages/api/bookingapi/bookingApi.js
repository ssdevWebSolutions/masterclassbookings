...
export async function createBooking(payload) {
  console.log(payload,"check consent");
  const res = await fetch(
...
export async function createCheckoutSession({ bookingId, amount, currency }) {
  ...
  console.log(amount,"price final");
...
========================= */
export async function recheckPayment(bookingId) {
  if (!bookingId) {
    throw new Error("Booking ID is required");
  }

  const res = await fetch(
    `${API_BASE}/training-class-bookings/${bookingId}/recheck-payment`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    }
  ...
  if (!res.ok) {
    const errorText = await res.text();
    throw new Error(errorText || "Failed to recheck payment");
  }

  return res.json(); // { bookingId, confirmed }
}
